...
sequenceDiagram
    ...
    DH->>DH: Response returns HTTP Status 200-299
...
* **Filter**: It filters requests containing `/wargame/challenges/` and `/auth/`. When a response returns, it checks if the HTTP status code is in the 2xx range (200-299, indicating successful authentication).
* **Signal**: If status code is in the 2xx range (200-299), it dispatches a `CustomEvent` named `DREAMHACK_CHALLENGE_SOLVED_EVENT` with the challenge name and ID.
