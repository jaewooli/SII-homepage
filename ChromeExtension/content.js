...
if (isDreamhack) {
  ...
  // DOM observer to detect already solved challenges
  let solveLogged = false;
  
  function checkForSolvedStatus() {
    if (solveLogged) return;
    
    // Check if the page contains the solved text "Solved"
    const allEls = document.querySelectorAll('*');
    let hasSolvedText = false;
    for (let el of allEls) {
      if (el.children.length === 0 && el.textContent.trim() === 'Solved') {
        hasSolvedText = true;
        break;
      }
    }
    ...
    if (hasSolvedText) {
      // Extract challenge ID from URL path (e.g. /wargame/challenges/931/)
      const match = window.location.pathname.match(/\/wargame\/challenges\/([^\/]+)/i);
      if (match) {
        const challengeId = match[1];
        solveLogged = true;
        
        let resolvedChallengeName = challengeId;
        try {
          const titleEl = document.querySelector('h1, h2, .challenge-title, [class*="title" i]');
          if (titleEl && titleEl.textContent) {
            resolvedChallengeName = titleEl.textContent.trim();
        ...
        console.log('[INHACK Extension] Already solved challenge detected via DOM! ID:', challengeId, 'Name:', resolvedChallengeName);
  ...
  // Set up a MutationObserver to watch for DOM updates (React rendering)
  const domObserver = new MutationObserver(() => {
    checkForSolvedStatus();
  });
  ...
  // Initial runs
  setTimeout(checkForSolvedStatus, 1000);
  setTimeout(checkForSolvedStatus, 3000);
}
