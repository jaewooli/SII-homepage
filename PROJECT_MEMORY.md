...
- [x] **관리자 계정 생성 및 권한 관리 이중화 (최고 관리자 vs 일반 관리자)**:
  ...
  * 직관적인 자연어로 버튼 명칭 변경 ("임명/일반화" ➡️ "관리자 지정/관리자 해제").
  * 이름 옆의 `관리자` 뱃지가 좁은 화면에서 가로 텍스트 말줄임(...) 처리에 밀려 잘리거나 전혀 보이지 않던 버그를 flex-shrink 배치가 적용된 견고한 인라인 레이아웃으로 리팩토링하여 해소.
  * `flex-basis`와 `min-width: 0` 등 Flexbox 규칙을 엄격하게 적용하여 뱃지가 추가되더라도 다른 이름 및 푼 문제 등의 컬럼 정렬 레이아웃이 찌그러지거나 뒤로 밀리지 않도록 고정.
