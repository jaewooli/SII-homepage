...
- [x] **관리자 계정 생성 및 권한 관리 이중화 (최고 관리자 vs 일반 관리자)**:
  ...
  * 최고 관리자 계정(`developer` 등)의 경우 `is_admin`/`isAdmin` 상태가 DB 필드 값과 상관없이 API 응답(/users, /me, /login)에서 언제나 참(`1`/`true`)으로 매핑되도록 보장하여, 다른 사용자 목록에서도 정상적으로 `최고` 및 `관리자` 텍스트 배지가 출력되도록 조치.
  * 사용자 목록을 불러올 때 최고 관리자 계정이 언제나 최상단(가장 위)에 위치할 수 있도록, SQLite 조회 쿼리에 **최고 관리자 정렬 우선순위 가중치 규칙(`ORDER BY (username = 'developer' OR username = ?) DESC, id DESC`)**을 도입하여 목록 정렬 규칙 개선.
  * 본인의 권한을 스스로 조정하거나 최고 관리자 및 타 일반 관리자 계정을 위협(차단, 삭제, 지정/해제)하는 것을 백엔드 수준에서 물리적으로 원천 차단하기 위해 **`/block-user` 및 `/delete-user` API 라우트에 데이터베이스 사전 대상 정보(is_admin) 검증 보호막 구축** 완료.
  * 프론트엔드에서도 이에 호응하여 **본인 행, 최고 관리자 행 및 (일반 관리자 로그인 시) 타 일반 관리자 행**에 대해서는 **`관리` 버튼 자체가 렌더링되지 않도록 강력한 가드 장치(`canManage`)**를 수립하고, 돔이 생성되지 않을 시 발생 가능한 자바스크립트 쿼리 예외 방어 처리 완료.
  * 본인의 권한을 스스로 조정하거나 최고 관리자의 권한을 박탈하는 것을 막기 위해, 사용자 목록 렌더링 시 **본인 행** 및 **최고 관리자 계정 행**에는 `관리자 지정/해제` 메뉴 옵션이 보이지 않도록 필터링 처리 구현.
  * 복잡했던 개별 액션 버튼들(차단, 삭제, 지정/해제)을 단 하나의 **`관리` 버튼 및 하위 드롭다운 팝오버 메뉴**로 통합하여 UI 컴팩트화 및 행 정렬 안정성을 비약적으로 향상.
    * 헤더와 바디의 가로 폭 패딩(좌우 8px)을 정확히 일치시켜 **`작업` 헤더와 `관리` 버튼의 세로 정렬 라인을 일렬로 일치**.
    * 드롭다운 내 메뉴 아이템 사이에 **구분선(Divider)**을 추가하고 옵션 성격별 **테마 색상(지정: 청색, 차단: 황색, 삭제: 적색) 및 호버 컬러**를 부여하여 시각적 인지 편의성 강화.
  * 아이디(Username) 컬럼의 화면 비중을 늘려(`flex: 1.5`), 긴 아이디도 잘림 없이 가독성이 확보되도록 테이블 컬럼 가로 배분비 최적화.
  * 관리자 계정임을 한눈에 식별할 수 있도록, **최고 관리자 계정 행에는 은은한 적색 배경과 빨간색 테두리(Border)**를, **일반 관리자 계정 행에는 은은한 청색 배경과 파란색 테두리(Border)**를 입히는 미래지향적 테두리 하이라이트 스타일 구현.
  * 역할 지정 관련 버튼 및 백엔드(admin.js)의 인가 실패 응답 메시지 용어 통일 ("임명/해임" ➡️ "지정/해제"로 일괄 변경하여 사용자 편의성 및 용어 통일성 확보).
  * 역할 배지(`최고`, `관리자`)는 아이디(Username)의 **왼쪽**에, 상태 배지(`차단됨`)는 아이디의 **오른쪽**에 배치하도록 구조를 재설계하여 최적의 가독성을 제공하며, 이름 옆에 두껍게 붙어 열 정렬을 깨트리던 이모지 표기(👑, 🛡️)를 완전히 폐기하고 깔끔한 미세 CSS 텍스트 뱃지(`최고` 빨강, `관리자` 파랑, `차단됨` 노랑)로 통일하여 이름(Name) 열 공간 100% 확보.
